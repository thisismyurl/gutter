 ...
 *
 * This is the ONE place a theme's identity lives. Change the `gutter_` function
 * prefix and the GUTTER_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `gutter_` function prefix and not a namespace (reversed 2026-07-20):
 * this line originally read `namespace Gutter;`, on the reasoning that callbacks
 ...
 * rewrites `gutter_` → `<slug>_`, `GUTTER_` → `<SLUG>_`, and `Gutter_` →
 ...
 * stays a string LITERAL ('gutter'), never the GUTTER_SLUG constant. `wp i18n
 ...
 */
define( 'GUTTER_VERSION', '1.6165.1612' );

/**
 * Absolute filesystem path to the theme root (no trailing slash).
 */
define( 'GUTTER_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'GUTTER_URI', get_template_directory_uri() );
