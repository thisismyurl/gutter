 ...
 *
 * Portable: every handle derives from the GUTTER_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the GUTTER_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function gutter_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'gutter_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function gutter_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( GUTTER_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( GUTTER_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'gutter_preload_fonts', 1 );
