 ...
 * copyright paragraph to the `gutter/copyright` source registered here (WP
 ...
 * names derive from GUTTER_SLUG, so a theme installed beside its siblings never
 ...
 */
function gutter_register_bindings(): void {
	if ( ! function_exists( 'register_block_bindings_source' ) ) {
		return;
	}

	register_block_bindings_source(
		GUTTER_SLUG . '/copyright',
		array(
			'label'              => esc_html__( 'Copyright line', 'gutter' ),
			'get_value_callback' => 'gutter_get_copyright_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		GUTTER_SLUG . '/site-strings',
		array(
			'label'              => esc_html__( 'Theme strings', 'gutter' ),
			'get_value_callback' => 'gutter_get_site_string_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		GUTTER_SLUG . '/publication-date',
		array(
			'label'              => esc_html__( 'Publication date', 'gutter' ),
			'get_value_callback' => 'gutter_get_publication_date_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		GUTTER_SLUG . '/footer-credit',
		array(
			'label'              => esc_html__( 'Footer credit', 'gutter' ),
			'get_value_callback' => 'gutter_get_footer_credit_value',
			'uses_context'       => array(),
		)
	);
}
add_action( 'init', 'gutter_register_bindings' );
 ...
 */
function gutter_get_site_string_value( array $source_args ): string {
	switch ( $source_args['key'] ?? '' ) {
		case 'breaking-label':
			return esc_html__( 'Breaking', 'gutter' );
		case 'home-label':
			return esc_html__( 'Back to the front page', 'gutter' );
		case 'home-url':
			return esc_url( home_url( '/' ) );
		case 'not-found-title':
			return esc_html__( 'Page not found', 'gutter' );
		case 'posts-title':
			$posts_page = (int) get_option( 'page_for_posts' );
			return $posts_page ? esc_html( get_the_title( $posts_page ) ) : esc_html__( 'Latest news', 'gutter' );
	}
	return '';
}
 ...
 */
function gutter_get_copyright_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( GUTTER_SLUG . '/copyright_date_format', 'Y' );
	$year   = (string) current_time( $format );

	$copyright = sprintf(
		/* translators: 1: four-digit year, 2: site title. */
		esc_html__( '© %1$s %2$s. All rights reserved.', 'gutter' ),
		$year,
	 ...
	 */
	$copyright = (string) apply_filters( GUTTER_SLUG . '/copyright_text', $copyright );
 ...
 */
function gutter_get_footer_credit_value(): string {
	$theme = wp_get_theme();
	$name  = $theme->get( 'Name' );
	...
	$text = sprintf(
		...
		esc_html__( 'Built with %s.', 'gutter' ),
		$credit
	);
	 ...
	 */
	$text = (string) apply_filters( GUTTER_SLUG . '/footer_credit_text', $text ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
 ...
 * flow gracefully".
 *
 * A gutter date is not interactive; it is the paper's dateline. Rendering it
 ...
 */
function gutter_get_publication_date_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( GUTTER_SLUG . '/publication_date_format', (string) get_option( 'date_format' ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
