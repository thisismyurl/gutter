 ...
 * from the theme, supplied through the `GUTTER_SLUG . '/get_started_content'` filter
 ...
 */
define( 'GUTTER_WELCOME_FLAG', 'gutter_welcome_notice' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
define( 'GUTTER_DISMISS_ACTION', 'gutter_dismiss_welcome' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-menu slug for the Get-started page.
 */
define( 'GUTTER_GET_STARTED_SLUG', 'gutter-get-started' );
 ...
 */
function gutter_flag_welcome_notice(): void {
	add_option( GUTTER_WELCOME_FLAG, '1', '', 'no' );
}
add_action( 'after_switch_theme', 'gutter_flag_welcome_notice' );
 ...
 */
function gutter_clear_welcome_notice(): void {
	delete_option( GUTTER_WELCOME_FLAG );
}

/**
 * Register the "Get started" page under Appearance.
 *
 * The required capability comes from gutter_get_onboarding_capability() — default is
 * edit_theme_options (users who can switch themes and configure global styles),
 * filterable via GUTTER_SLUG . '/onboarding_capability'.
 */
function gutter_register_get_started_page(): void {
	add_theme_page(
		/* translators: %s: theme name. */
		sprintf( esc_html__( '%s: Get started', 'gutter' ), gutter_get_theme_name() ),
		gutter_get_theme_name(),
		gutter_get_onboarding_capability(),
		GUTTER_GET_STARTED_SLUG,
		'gutter_render_get_started_page'
	);
}
add_action( 'admin_menu', 'gutter_register_get_started_page' );
 ...
 */
function gutter_get_theme_name(): string {
 ...
 */
function gutter_get_onboarding_capability(): string {
	/**
	 ...
	 */
	return (string) apply_filters( GUTTER_SLUG . '/onboarding_capability', 'edit_theme_options' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function gutter_enqueue_get_started_assets( string $hook_suffix ): void {
	if ( 'appearance_page_' . GUTTER_GET_STARTED_SLUG !== $hook_suffix ) {
		return;
	}

	$sheet = GUTTER_DIR . '/assets/css/admin-get-started.css';
	if ( ! file_exists( $sheet ) ) {
		return;
	}

	wp_enqueue_style(
		GUTTER_SLUG . '-get-started',
		GUTTER_URI . '/assets/css/admin-get-started.css',
		array(),
		(string) filemtime( $sheet )
	);
}
add_action( 'admin_enqueue_scripts', 'gutter_enqueue_get_started_assets' );

/**
 * Show the dismissible welcome notice while the flag is set.
 *
 * Shown only to users who pass gutter_get_onboarding_capability(), and never on the Get-started
 ...
 */
function gutter_render_welcome_notice(): void {
	if ( ! current_user_can( gutter_get_onboarding_capability() ) ) {
		return;
	}

	if ( '1' !== get_option( GUTTER_WELCOME_FLAG ) ) {
		return;
	}

	$screen = get_current_screen();
	if ( $screen instanceof \WP_Screen && 'appearance_page_' . GUTTER_GET_STARTED_SLUG === $screen->id ) {
		return;
	}

	$page_url = admin_url( 'themes.php?page=' . GUTTER_GET_STARTED_SLUG );
	$theme    = gutter_get_theme_name();
	$dismiss  = wp_nonce_url( admin_url( 'admin-post.php?action=' . GUTTER_DISMISS_ACTION ), GUTTER_DISMISS_ACTION );
	?>
...
add_action( 'admin_notices', 'gutter_render_welcome_notice' );
 ...
 */
function gutter_handle_welcome_dismiss(): void {
	if ( ! current_user_can( gutter_get_onboarding_capability() ) ) {
		wp_die( esc_html__( 'You do not have permission to do that.', 'gutter' ) );
	}

	check_admin_referer( GUTTER_DISMISS_ACTION );
	gutter_clear_welcome_notice();
	wp_safe_redirect( admin_url() );
	exit;
}
add_action( 'admin_post_' . GUTTER_DISMISS_ACTION, 'gutter_handle_welcome_dismiss' );

/**
 * The Get-started page content.
 *
 * Core supplies a plain, honest default so the page is coherent out of the box.
 * A theme overrides any part of it through the `GUTTER_SLUG . '/get_started_content'`
 ...
 */
function gutter_get_started_content(): array {
	$theme = gutter_get_theme_name();

	$default = array(
		'lead'       => sprintf(
			/* translators: %s: theme name. */
			esc_html__( '%s is a free, full-site-editing theme built to get out of the way of your content. Here is how to make it yours.', 'gutter' ),
			$theme
		),
		/*
		 ...
		 * echo in gutter_render_get_started_page() (the 'title' at the <strong>, the
		...
		'steps'      => array(
			array(
				...
				'body'  => esc_html__( "A visitor's first second decides whether they stay. Set a static front page under Settings → Reading, and give your posts a page of their own.", 'gutter' ),
			),
			array(
				'title' => __( 'Give people a way around.', 'gutter' ),
				'body'  => esc_html__( 'A site without a menu is a room without doors. Open the Site Editor, edit the header, and assign your menu to Primary Navigation.', 'gutter' ),
			),
			array(
				'title' => __( 'Start from a pattern, not a blank page.', 'gutter' ),
				'body'  => esc_html__( 'In any page or post, open the block inserter, choose Patterns, and find this theme\'s group. Drop one in and change the words.', 'gutter' ),
			),
			array(
				'title' => __( 'Make it sound like you.', 'gutter' ),
				'body'  => esc_html__( 'In the Site Editor, open Styles to change colours and typefaces. Nothing you do there can break the theme — experiment freely.', 'gutter' ),
			),
		),
		'optimize'   => array(
			esc_html__( "This theme is fast by design: reading is JavaScript-free (only the breaking-news dismiss control ships a small script), self-hosted fonts that don't phone home, and tuning against the Core Web Vitals search engines actually measure.", 'gutter' ),
			esc_html__( 'It is built to WCAG 2.2 AA guidance — real focus outlines, a skip link, sensible heading order, and motion that respects a reduce-motion setting. Keep your own copy and images to that bar and the whole site stays welcoming.', 'gutter' ),
		),
		'developers' => array(
			...
			'url'   => apply_filters( GUTTER_SLUG . '/developer_guide_url', 'https://thisismyurl.com/gutter/' ), // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
	 ...
	 *
	 * A theme replaces any key with its own voice. See gutter_get_started_content() for
	 * the array shape.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	return (array) apply_filters( GUTTER_SLUG . '/get_started_content', $default ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function gutter_render_get_started_page(): void {
	if ( ! current_user_can( gutter_get_onboarding_capability() ) ) {
		return;
	}
	...
	$theme   = gutter_get_theme_name();
	$content = gutter_get_started_content();
